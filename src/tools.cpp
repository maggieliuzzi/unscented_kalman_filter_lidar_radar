...
{
	UKF ukf = car.ukf;
	viewer->addSphere(pcl::PointXYZ(ukf.state_[0],ukf.state_[1],3.5), 0.5, 0, 1, 0,car.name+"_ukf");
	viewer->addArrow(pcl::PointXYZ(ukf.state_[0], ukf.state_[1],3.5), pcl::PointXYZ(ukf.state_[0]+ukf.state_[2]*cos(ukf.state_[3]),ukf.state_[1]+ukf.state_[2]*sin(ukf.state_[3]),3.5), 0, 1, 0, car.name+"_ukf_vel");
	if (time > 0)
	{
		...
		{
			ukf.Prediction(dt);
			viewer->addSphere(pcl::PointXYZ(ukf.state_[0],ukf.state_[1],3.5), 0.5, 0, 1, 0,car.name+"_ukf"+std::to_string(ct));
			...
			//viewer->addArrow(pcl::PointXYZ(ukf.state_[0], ukf.state_[1],3.5), pcl::PointXYZ(ukf.state_[0]+ukf.state_[2]*cos(ukf.state_[3]),ukf.state_[1]+ukf.state_[2]*sin(ukf.state_[3]),3.5), 0, 1, 0, car.name+"_ukf_vel"+std::to_string(ct));
...
VectorXd Tools::CalculateRMSE(const vector<VectorXd> &estimations, const vector<VectorXd> &ground_truth)  // root mean squared error
{
    VectorXd rmse(4);
	rmse << 0,0,0,0;
	...
	// calculate the mean of sum of residuals/ error
	rmse = rmse/estimations.size();

	// calculate the squared root of the mean of the sum of residuals
